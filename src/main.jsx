...
function StrengthForm({workout,value,setData,date}){
  ...
  const openLastTime=async(ex)=>{
    ...
    try{
      const result=await api(`/logs/exercise/${encodeURIComponent(ex.id)}?before=${encodeURIComponent(date)}`);
  ...
  const copyPrevious=(ex,si)=>{
    if(si<=0) return;
    const sets=exerciseSets(ex);
    sets[si]={...normalizeSet(sets[si-1])};
    setData(['exercises',ex.id,'sets'],sets);
  };
      ...
      <div className="last-time-modal">
        ...
        {lastError ? <p className="last-time-empty">{lastError}</p> : <>
          ...
          <div className="last-time-sets">
            {(lastExercise.entry?.sets||[]).map((set,i)=>{
              const normalized=normalizeSet(set);
              return <div className="last-time-set" key={i}><span>Set {i+1}</span><strong>{normalized.weight === '' ? '—' : `${normalized.weight} kg`}</strong><strong>{normalized.reps === '' ? '—' : `${normalized.reps} reps`}</strong></div>;
            })}
          </div>
